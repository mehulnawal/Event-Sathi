...
const StatCard = ({ label, count, color, href, icon }) => (
  <Link href={href}>
    <div
      className={`bg-white rounded-xl border p-4 sm:p-6 flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer ${color}`}
    >
      <div className="text-2xl">{icon}</div>
      <div>
        <p className="text-xl sm:text-2xl font-bold text-[#1C1C1C]">{count}</p>
        ...
        <thead>
          <tr className="bg-[#F5F0E8]">
            {columns.map((col) => (
              <th
                key={col}
                className="text-left px-4 py-2.5 text-xs font-bold text-[#8C7B6B] uppercase tracking-wider whitespace-nowrap"
              >
                {col}
              </th>
            ))}
          ...
          {rows.length === 0 ? (
            <tr>
              <td
                ...
                className="px-4 py-6 text-center text-sm text-[#8C7B6B] whitespace-nowrap"
              >
                No entries yet
              </td>
            </tr>
            ...
            rows.map((row, i) => (
              <tr
                key={i}
                className="border-t border-[#F5F0E8] hover:bg-[#FDFAF5]"
              >
                {row.map((cell, j) => (
                  <td
                    key={j}
